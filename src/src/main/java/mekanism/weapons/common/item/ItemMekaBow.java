...
@Nonnull
   ...
   public void onUsingTick(ItemStack stack, EntityLivingBase player, int count) {
    // --- LOGICA AUTO-FIRE (SOLO SERVER) ---
    if (!player.world.isRemote && isModuleEnabled(stack, MekanismWeaponsModules.AUTO_FIRE_UNIT)) {
        if ((getMaxItemUseDuration(stack) - count) >= getDrawTicks(stack)) {
            player.stopActiveHand();
            return; // Esci per evitare di eseguire altro codice
        }
    }

    // --- LOGICA DI FORZATURA ANIMAZIONE (SOLO CLIENT) ---
    // Questo è il pezzo che risolve il problema visivo.
    if (player.world.isRemote) {
        // Leggiamo il tempo di carica corretto...
        int ticks = getDrawTicks(stack);
        // ...e lo scriviamo in un tag NBT personalizzato ogni singolo tick.
        // Questo forza il client a usare sempre il valore aggiornato per l'animazione.
        ItemDataUtils.setInt(stack, "animDrawTicks", ticks);
    }
}
   ...
   public void onPlayerStoppedUsing(ItemStack stack, World worldIn, EntityLivingBase entityLiving, int timeLeft) {
    ...
    // Il resto del tuo codice per calcolare l'energia e sparare la freccia
    // è già corretto e può rimanere invariato.
