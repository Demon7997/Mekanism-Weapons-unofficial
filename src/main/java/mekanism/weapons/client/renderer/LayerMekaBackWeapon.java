...
public class LayerMekaBackWeapon implements LayerRenderer<EntityPlayer> {
    ...
    public void doRenderLayer(EntityPlayer player, float limbSwing, float limbSwingAmount, float partialTicks, float ageInTicks, float netHeadYaw, float headPitch, float scale) {
        ...
        ItemStack magnetizer = findMagnetizerInBaubles(player);
    
        if (magnetizer.isEmpty()) return;
        
        renderOnBack(player, magnetizer, 0.0, 0.42, -0.13, 180.0f);
    ...
    public boolean shouldCombineTextures() { return false; }
        ...
        private ItemStack findMagnetizerInBaubles(EntityPlayer player) {
        try {
            baubles.api.cap.IBaublesItemHandler handler = BaublesApi.getBaublesHandler(player);
            if (handler != null) {
